...
class Model:
    def __init__(self, config, is_train=True, embedding_init_value=None, num_of_layer=1, pre_train_mode=True):
        ...
        forward_encoder = tf.contrib.rnn.MultiRNNCell(
            [tf.contrib.rnn.LSTMCell(config.UNIT_SIZE,
                                     state_is_tuple=True,
                                     forget_bias=1.0,
        ...
        if pre_train_mode:
            if is_train:
                ...
                self.dec_first_input_index = tf.placeholder(shape=[None], dtype=tf.int32)
                batch_size = tf.shape(self.dec_first_input_index)[0]
                dec_first_input = tf.nn.embedding_lookup(embedding, self.dec_first_input_index)
            self.initial_dec_state = decoder.zero_state(batch_size, tf.float32)
